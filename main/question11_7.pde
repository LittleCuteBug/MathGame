class ControlButton11_7 extends RectButton{
    int labalSize = 5;
    
    String type;
    
    ControlButton11_7(String _text, float _x, float _y) {
        super(_text, _x, _y, 15.0, 10.0, 10.0);
    }
    
    ControlButton11_7(String _text, float _x, float _y, float _text_size) {
        super(_text, _x, _y, _text_size, _text_size * 0.8, _text_size * 0.8);
    }
    
    ControlButton11_7(String _text, float _x, float _y, float _text_size, String _type) {
        super(_text, _x, _y, _text_size, _text_size * 0.8, _text_size * 0.8);
        type = _type;
    }
    ...
    void draw() {
        ...
        if(type.equals("right")) {
            ...
            line(x - x_radius, y + y_radius, x + x_radius, y - y_radius);
            line(x, y - y_radius, x + x_radius, y - y_radius);
            line(x + x_radius, y, x + x_radius, y - y_radius);
...
class Answer11_7 {
    ...
    Answer11_7(int _ordinal, String _sequence) {
        ordinal = _ordinal;
        sequence = _sequence;
    ...
    void draw() {
        ...
        //draw border
        for(int i = 0; i < 2; i ++) {
            rect(pos_x + i * segment, pos_y, segment, segment);
            line(pos_x + i * segment, pos_y + segment, pos_x + i * segment + segment, pos_y);
        }
        
        //draw painted
        int cur_x = pos_x;
        int cur_y = pos_y + segment;
        ...
        for(int i = 0; i < sequence.length(); i ++) {
            ...
            if(sequence.charAt(i) == 'R') {
                ...
                line(cur_x, cur_y, cur_x + segment, cur_y - segment);
                cur_x = cur_x + segment;
                cur_y = cur_y - segment;
            }
        }
...
class MainModel11_7 {
    ...
    PImage spider = loadImage("../data/img/spider.png");
    PImage pond = loadImage("../data/img/pond.png");
    
    int x_end = x_start + 2 * segment;
    int y_end = y_start - 1 * segment;
    ...
    int counter = 0;
    String sequence = "";  
    ArrayList<Answer11_7> listAnswer = new ArrayList<Answer11_7>();
    String[] solution = {"URR", "CR", "RUR", "RC", "RRU"};
    ...
    boolean button_solution_clicked = false;
    ...
    void draw() {
        ...
        strokeWeight(border_size);
        for(int i = 0; i < 2; i ++) {
            rect(x_start + i * segment, y_start - segment, segment, segment);
            line(x_start + i * segment, y_start, x_start + i * segment + segment, y_start - segment);
        }
        ...
        //draw answer
        for(Answer11_7 answer : listAnswer) {
            answer.draw();
        }
        ...
        if(counter > 0) {
            ...
            //moving cross
            if(c_left > 0) {
                for(int s = 0; s < speed && c_left > 0; s ++) {
                    c_left --;
                    x_cur ++;
                    y_cur --;
                    x_move = append(x_move, x_cur);
        ...
        if(x_cur == x_end && y_cur == y_end) {
            ...
            if(check) {
                Answer11_7 newAnswer = new Answer11_7(listAnswer.size(), sequence);
        ...
        if(counter == 0) {
            ...
            if(x_cur < x_end && y_cur > y_end) {
                strokeWeight(border_size);
                line(x_cur, y_cur, x_cur + segment, y_cur - segment);
                strokeWeight(arrow_size);
                for(int i = count_period; i < segment; i += period) {
                    line(x_cur + i, y_cur - i, x_cur + i - period / 2, y_cur - i);
                    line(x_cur + i, y_cur - i, x_cur + i, y_cur - i + period / 2);
                }

                strokeWeight(dot_size);
                point(x_cur + segment, y_cur - segment);
            }
        ...
        //draw spider
        int spider_size = 80;
        image(spider, x_cur - spider_size / 2, y_cur - spider_size / 3, spider_size, 2 * spider_size / 3);
        ...
        if(button_solution.clicked()) {
            ...
            if(counter <= 0) {
                for(String _solution : solution) {
                    ...
                    if(check) {
                        Answer11_7 newAnswer = new Answer11_7(listAnswer.size(), _solution);
                        listAnswer.add(newAnswer);
                    }
                }
            }
    ...
    boolean isFinish() {
        return listAnswer.size() == solution.length && !button_solution_clicked;
    }
}

class Question11_7 extends Question { 
    MainModel11_7 model = new MainModel11_7();
    
    void setup() {
        super.setup();  
        String quizText = "Một con nhện đang ở vị trí A. Hỏi có bao nhiêu cách để nó có thể quay trở về vị trí B? Biết chú ta chỉ có thể sử dụng 3 hướng đi lên, phải và chéo.";
