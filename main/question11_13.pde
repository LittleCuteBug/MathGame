class ControlButton11_13 extends RectButton{
    int labalSize = 5;
    
    String type;
    
    ControlButton11_13(String _text, float _x, float _y) {
        super(_text, _x, _y, 15.0, 10.0, 10.0);
    }
    
    ControlButton11_13(String _text, float _x, float _y, float _text_size) {
        super(_text, _x, _y, _text_size, _text_size * 0.8, _text_size * 0.8);
    }
    
    ControlButton11_13(String _text, float _x, float _y, float _text_size, String _type) {
        super(_text, _x, _y, _text_size, _text_size * 0.8, _text_size * 0.8);
        type = _type;
    }
...
class Answer11_13 {
    ...
    Answer11_13(int _ordinal, String _sequence) {
        ...
        pos_x = pos_x + ordinal % 3 * (segment * 3 + distance);
        pos_y = pos_y + ordinal / 3 * (segment * 3 + distance);
    }
...
class MainModel11_13 {
    ...
    PImage anteater = loadImage("../data/img/anteater.png");
    
    int x_end = x_start + 3 * segment;
    int y_end = y_start + 3 * segment;
    ...
    int counter = 0;
    String sequence = "";  
    ArrayList<Answer11_13> listAnswer = new ArrayList<Answer11_13>();
    String[] solution = {"DDDRRR", "DDRDRR", "DRDDRR", "RDDDRR", "RRRDDD", "RRDRDD", "RDRRDD", "DRRRDD"};
    ...
    boolean block(int x, int y) {
        return(x == x_start + 2 * segment && y == y_start + 2 * segment);
    }
    ...
    void draw() {
        ...
        if(x_cur == x_end && y_cur == y_end) {
            ...
            if(check) {
                Answer11_13 newAnswer = new Answer11_13(listAnswer.size(), sequence);
        ...
        // //draw start
        int pond_size = 100;
        image(pond, x_start - pond_size - 15, y_start - pond_size / 2, pond_size, pond_size);
        fill(0);
        textSize(50);
        text("A", x_start + 40, y_start - 10);
        textSize(12);
        ...
        //draw block anteater
        int anteater_size = 80;
        image(anteater, x_start + 2 * segment - anteater_size / 2, y_start + 2 * segment - anteater_size / 2, anteater_size, anteater_size);
        ...
        if(button_solution.clicked()) {
            ...
            if(counter <= 0) {
                for(String _solution : solution) {
                    ...
                    if(check) {
                        Answer11_13 newAnswer = new Answer11_13(listAnswer.size(), _solution);
                        listAnswer.add(newAnswer);
                    }
                }
            }
...
class Question11_13 extends Question { 
    MainModel11_13 model = new MainModel11_13();
