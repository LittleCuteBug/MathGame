class Question12_9 extends Question {
  ...
  void setup() {
    super.setup();
    String quizText = "Câu 12_9: Melvin và Edward có 80 đô-la. Edward có nhiều hơn Melvin 10 đô-la. Hỏi mỗi bạn có bao nhiêu đô-la?";
  ...
  void draw() {
    ...
    fill(0);
    text("Edward có", 300, posYAnswerBoxTitle);
    text(answer[0], 305, posYAnswerBoxText);
    text("Melvin có", 500, posYAnswerBoxTitle);
    ...
    if (sol) {
      ...
      text("Edward có: (" + str(big + small) + " + " + str(big - small) + ")" + " / 2 = " + str(big) + ". " + "Melvin có: " + str(big) + " - " + str(big - small) + " = " + str(small) + ".", 180, 420);
    }
    if (curColor == 255) {
      curSum = big * 2;
    }
